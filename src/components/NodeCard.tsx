...
export const NodeCard: React.FC<NodeCardProps> = ({
  ...
  const getStatusBadge = () => {
    switch (node.status) {
      ...
      case 'issue':
        return (
          <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-medium bg-rose-100 text-rose-800 border border-rose-200">
  ...
  return (
    <div
      ...
      onClick={() => onEdit(node)}
      className={`group relative bg-white border rounded-xl p-3 shadow-sm hover:shadow-md transition-all duration-200 cursor-pointer select-none min-h-[90px] flex flex-col justify-between ${
        node.isKey
    ...
    >
      ...
      <div className="flex items-start justify-between gap-1 mb-1.5">
        <div className="flex items-center gap-1.5 flex-wrap">
          {node.isKey && (
            <span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[10px] font-bold bg-amber-500 text-white shadow-xs">
      ...
      {/* Main Title */}
      <div className="text-xs font-bold text-slate-800 leading-snug mb-1 group-hover:text-blue-600 transition-colors">
        {node.title}
      </div>
      ...
      <div className="flex items-center justify-between pt-1 mt-auto border-t border-slate-100 text-[10px] text-slate-400">
        ...
        {onStartConnect && (
          <button
            ...
            className="opacity-0 group-hover:opacity-100 p-1 hover:bg-slate-100 text-slate-500 hover:text-blue-600 rounded transition-all flex items-center gap-0.5"
            title="连线到其他节点"
          >
