...
export const FlowBoardView: React.FC<FlowBoardViewProps> = ({ data, onEditNode }) => {
  return (
    <div className="p-6 bg-slate-900 min-h-[calc(100vh-64px)] text-slate-100 overflow-x-auto">
      <div id="journey-export-canvas" className="journey-canvas-area max-w-[1920px] mx-auto p-4 bg-slate-900 rounded-2xl">
        <div className="mb-6">
                ...
                <div className="space-y-4">
                  {stageSubStages.map((subStage) => {
                    ...
                    return (
                      ...
                      >
                        <div className="text-sm font-semibold text-blue-400 mb-2.5 pb-1 border-b border-slate-800 flex items-center justify-between">
                          <span className="flex items-center gap-1">
                            {subStage.isKey && <Star className="w-4 h-4 text-amber-400 fill-amber-400" />}
                            {subStage.name}
                          </span>
                          <span className="text-xs text-slate-400">{subStageNodes.length} 个动作</span>
                        </div>
                        ...
                        <div className="space-y-2">
                          {subStageNodes.length > 0 ? (
                            subStageNodes.map((node) => {
                              ...
                              return (
                                <div
                                  ...
                                  className="bg-slate-800 hover:bg-slate-750 border border-slate-700 hover:border-blue-500 rounded-xl p-3 cursor-pointer transition-all shadow-xs"
                                >
                                  ...
                                  <div className="text-sm font-bold text-slate-100 mb-1">{node.title}</div>
                            ...
                            <div className="text-sm text-slate-500 italic p-2 text-center">暂无节点</div>
                          )}
                        </div>
                      </div>
                    );
